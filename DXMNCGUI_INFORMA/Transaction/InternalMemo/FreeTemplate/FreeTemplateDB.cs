...
namespace DXMNCGUI_INFORMA.Transaction.InternalMemo.FreeTemplate
{
    public class FreeTemplateDB
    {
        ...
        {
            return this.myDBReg.IncOne((IRegistryID)new FreeTemplateHeaderDocKey());
        }
        public long DtlAppKeyUniqueKey()
        {
            return this.myDBReg.IncOne((IRegistryID)new FreeTemplateApprovalDtlKey());
        }

        public static FreeTemplateDB Create(SqlDBSetting dbSetting, SqlLocalDBSetting localdbsetting, SqlDBSession dbSession)
        {
            FreeTemplateDB aFreeTemplateDB = (FreeTemplateDB)null;
            aFreeTemplateDB = new FreeTemplateSql();
            aFreeTemplateDB.myDBSetting = dbSetting;
            aFreeTemplateDB.myLocalDBSetting = localdbsetting;
            aFreeTemplateDB.myDBSession = dbSession;
            return aFreeTemplateDB;
        }
        ...
        public FreeTemplateEntity Entity(DXIType type, string sDocTypeValue)
        {
            ...
            return new FreeTemplateEntity(this, dataSet, DXIAction.New);
        }
        public FreeTemplateEntity GetEntity(long headerid, string docno)
        {
            myDBReg = DBRegistry.Create(myLocalDBSetting);
            ...
            if (ds.Tables[0].Rows.Count == 0)
                return null;
            return new FreeTemplateEntity(this, ds, DXIAction.Edit);
        }
        public FreeTemplateEntity View(long headerid, string docno, DXIAction action)
        {
            myDBReg = DBRegistry.Create(myLocalDBSetting);
            return this.InternalView(this.LoadData(headerid, docno));
        }
        public FreeTemplateEntity Edit(long headerid, string docno, DXIAction action)
        {
            myDBReg = DBRegistry.Create(myLocalDBSetting);
            return this.InternalEdit(this.LoadData(headerid, docno));
        }
        public FreeTemplateEntity Approve(long headerid, string docno)
        {
            myDBReg = DBRegistry.Create(myLocalDBSetting);
            return this.InternalApprove(this.LoadData(headerid, docno));
        }

        private FreeTemplateEntity InternalView(DataSet newDataSet)
        {
            if (newDataSet.Tables["Header"].Rows.Count == 0)
            {
                return (FreeTemplateEntity)null;
            }
            else
            {
                long docKey = Convert.ToInt64(newDataSet.Tables["Header"].Rows[0]["DocKey"]);
                return new FreeTemplateEntity(this, newDataSet, DXIAction.View);
            }
        }
        private FreeTemplateEntity InternalEdit(DataSet newDataSet)
        {
            if (newDataSet.Tables["Header"].Rows.Count == 0)
            {
                return (FreeTemplateEntity)null;
            ...
            {
                long docKey = Convert.ToInt64(newDataSet.Tables["Header"].Rows[0]["DocKey"]);
                return new FreeTemplateEntity(this, newDataSet, DXIAction.Edit);
            }
        }
        private FreeTemplateEntity InternalApprove(DataSet newDataSet)
        {
            if (newDataSet.Tables["Header"].Rows.Count == 0)
            {
                return (FreeTemplateEntity)null;
            ...
            {
                long docKey = Convert.ToInt64(newDataSet.Tables["Header"].Rows[0]["DocKey"]);
                return new FreeTemplateEntity(this, newDataSet, DXIAction.Approve);
            }
        }
        ...
        public void SaveEntity(FreeTemplateEntity entity, string strDocName, string strUserID, DXISaveAction saveaction, string strApprovalNote)
        {
            if (entity.DocNo.ToString().Length == 0)
                throw new FreeTemplateCodeException();
            ...
            try
            {
                if (myBrowseTable.Rows.Count > 0)
                {
                    ...
                    if (r == null)
                    {
                        ...
                        foreach (DataColumn col in entity.DataTableHeader.Columns)
                        {
        ...
        protected virtual void SaveData(FreeTemplateEntity InternalMemo, DataSet ds, string strDocName, string strUserID, DXISaveAction saveaction, string strApprovalNote)
        {
        }
    }
}